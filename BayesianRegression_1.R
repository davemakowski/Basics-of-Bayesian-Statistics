...
plot(TAB$TGS, TAB$Sensitivity, xlab="Mean temperature (°C)", ylab="% yield difference", pch=19, ylim=c(-25,20), col="blue")
title("Individual yield sensitivity values vs. Site average temperature")
abline(h=0, lty=2)
...
#Mean values and confidence intervals by site
dev.new()
par(mfrow=c(1,2))
...
plot(TGSm[,2], Mean[,2], xlab="Mean temperature (°C)", ylab="% yield difference", pch=19, ylim=c(-25,20), col="blue")
for (i in 1:(dim(Mean)[1])) {
	lines(c(TGSm[i,2], TGSm[i,2]), c(Mean[i,2]-1.96*SD[i,2]/sqrt(Nobs[i,2]),Mean[i,2]+1.96*SD[i,2]/sqrt(Nobs[i,2])))
}
title("Yield sensitivities to +1°C vs. Site average temperature")
